...
class VaksinatorFhwModel extends CI_Model{
    ...
    private $listdusun = ['vaksinator1'=>array("Lekor+Timur"=>"Lekor Timur","Lekor+Barat"=>"Lekor Barat","Sondo"=>"Sondo","Gulung"=>"Gulung","Pepao+Timur"=>"Pepao Timur","Lengkok+Bunut"=>"Lengkok Bunut","Lendang+Jawe"=>"Lendang Jawe","Pepao+Barat+I"=>"Pepao Barat I","Presak"=>"Presak","Taken-Aken"=>"Taken Aken","Pepao+Barat+II"=>"Pepao Barat II","Renge"=>"Renge","Pelapak"=>"Pelapak","Walun"=>"Walun","Montong+Bile"=>"Montong Bile","Lainnya"=>"Lainnya")
                        ...
                        ,'vaksinator14'=>array("Anak+Anjan"=>"Anak Anjan","Penupi"=>"Penupi","Kadik+I"=>"Kadik I","Karang+baru"=>"Karang Baru","Tenang"=>"Tenang","Lamben"=>"Lamben","Tuban"=>"Tuban","Segale"=>"Segale","Tenang+Baru"=>"Tenang Baru","Kadik+II"=>"Kadik II","Anak Anjan"=>"Anak Anjan","Kadik II"=>"Kadik II","Dasan Duah"=>"Dasan Duah","Lainnya"=>"Lainnya")];
    ...
    public function getCountPerForm($desa=""){
        ...
        foreach ($tables as $table=>$legend){
            ...
            if($table=="registrasi_jurim"){
                $query = $vaksinatorDB->query("SELECT userid, DATE(clientversionsubmissiondate) as submissiondate,village as dusun,count(*) as counts from ".$table." where (userid='$username') group by village");
                ...
                foreach ($query->result() as $c_data){
                    $query2 = $vaksinatorDB->query("SELECT village as dusun FROM registrasi_jurim where childId='$c_data->childId'");
    ...
    public function getCountPerDay($desa="",$mode=""){
        ...
        foreach ($tables as $table){
            ...
            if($table=="registrasi_jurim"){
                $query = $vaksinatorDB->query("SELECT userid, DATE(clientversionsubmissiondate) as submissiondate,village as dusun,count(*) as counts from ".$table." where (userid='$username') group by village,DATE(clientversionsubmissiondate)");
                foreach ($query->result() as $datas){
                    if(array_key_exists($datas->dusun, $namadusun)){
                        $data_count                  = $result_data[$namadusun[$datas->dusun]];
    ...
    public function getCountPerMode($desa="",$mode="Mingguan"){
        ...
        foreach ($tables as $table){
            ...
            if($table=="registrasi_jurim"){
                if($mode=='Mingguan'){
                    $query = $vaksinatorDB->query("SELECT userid, DATE(clientversionsubmissiondate) as submissiondate,village as dusun,count(*) as counts from ".$table." where (userid='$username') and (DATE(clientversionsubmissiondate) >= '".date("Y-m-d",  strtotime((!(date('N', strtotime($now)) >= 6)?"last Saturday ":"")."-5 days"))."' and DATE(clientversionsubmissiondate) <= '".date("Y-m-d",  strtotime((!(date('N', strtotime($now)) >= 6)?"next Saturday ":"")))."') group by village, DATE(clientversionsubmissiondate)");
                }elseif($mode=='Bulanan'){
                    $query = $vaksinatorDB->query("SELECT userid, DATE(clientversionsubmissiondate) as submissiondate,village as dusun,count(*) as counts from ".$table." where (userid='$username') and (DATE(clientversionsubmissiondate) >= '".date("Y-m",strtotime("+".(-$this_month-11)." months"))."' and DATE(clientversionsubmissiondate) <= '".date("Y-m",strtotime("+".(12-$this_month)." months"))."') group by village, DATE(clientversionsubmissiondate)");
                }
                foreach ($query->result() as $datas){
                    if($mode=='Mingguan'){
                        if(array_key_exists($datas->dusun, $namadusun)){
