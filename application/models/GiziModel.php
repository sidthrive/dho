...
class GiziModel extends CI_Model{
    ...
    public function getCountPerDay($kecamatan="",$mode="",$range=""){
       ...
       if($kecamatan=='Sengkol'){
        ...
        foreach ($tables as $table=>$legend){
            ...
            if($range!=""){
                if($kecamatan=='Sengkol'){
                    $query = $giziDB->query("SELECT userid, DATE(clientversionsubmissiondate) as submissiondate,count(*) as counts from ".$table." where (userid='gizi8' or userid='gizi9' or userid='gizi10' or userid='gizi11' or userid='gizi12' or userid='gizi13' or userid='gizi14') and (DATE(clientversionsubmissiondate) >= '".$range[0]."' and DATE(clientversionsubmissiondate) <= '".$range[1]."') group by userid, DATE(clientversionsubmissiondate)");
                }
                elseif($kecamatan=='Janapria'){
                    $query = $giziDB->query("SELECT userid, DATE(clientversionsubmissiondate) as submissiondate,count(*) as counts from ".$table." where (userid='gizi1' or userid='gizi2' or userid='gizi3' or userid='gizi4' or userid='gizi5' or userid='gizi6') and (DATE(clientversionsubmissiondate) >= '".$range[0]."' and DATE(clientversionsubmissiondate) <= '".$range[1]."') group by userid, DATE(clientversionsubmissiondate)");
                }
            }else{
                if($kecamatan=='Sengkol'){
    ...
    public function getCountPerDayByVisitDate($kecamatan="",$mode="",$range=""){
       ...
       if($kecamatan=='Sengkol'){
        ...
        foreach ($tables as $table=>$legend){
            ...
            if($range!=""){
                if($kecamatan=='Sengkol'){
                    $query = $giziDB->query("SELECT userid, tanggalPenimbangan as submissiondate,count(*) as counts from ".$table." where (userid='gizi8' or userid='gizi9' or userid='gizi10' or userid='gizi11' or userid='gizi12' or userid='gizi13' or userid='gizi14') and (tanggalPenimbangan >= '".$range[0]."' and tanggalPenimbangan <= '".$range[1]."') group by userid, tanggalPenimbangan");
                }
                elseif($kecamatan=='Janapria'){
                    $query = $giziDB->query("SELECT userid, tanggalPenimbangan as submissiondate,count(*) as counts from ".$table." where (userid='gizi1' or userid='gizi2' or userid='gizi3' or userid='gizi4' or userid='gizi5' or userid='gizi6') and (tanggalPenimbangan >= '".$range[0]."' and tanggalPenimbangan <= '".$range[1]."') group by userid, tanggalPenimbangan");
                }
            }else{
                if($kecamatan=='Sengkol'){
                    $query = $giziDB->query("SELECT userid, tanggalPenimbangan as submissiondate,count(*) as counts from ".$table." where (userid='gizi8' or userid='gizi9' or userid='gizi10' or userid='gizi11' or userid='gizi12' or userid='gizi13' or userid='gizi14') and (tanggalPenimbangan >= '".date("Y-m-d",strtotime("-30 days"))."' and tanggalPenimbangan <= '".date("Y-m-d")."') group by userid, tanggalPenimbangan");
                }
                elseif($kecamatan=='Janapria'){
                    $query = $giziDB->query("SELECT userid, tanggalPenimbangan as submissiondate,count(*) as counts from ".$table." where (userid='gizi1' or userid='gizi2' or userid='gizi3' or userid='gizi4' or userid='gizi5' or userid='gizi6') and (tanggalPenimbangan >= '".date("Y-m-d",strtotime("-30 days"))."' and tanggalPenimbangan <= '".date("Y-m-d")."') group by userid, tanggalPenimbangan");
                }
            }
    ...
    public function getCountPerFormByVisitDate($kecamatan="",$start,$end){
        ...
        foreach ($table_default as $table=>$legend){
            ...
            //query tha data
            if($kecamatan=='Sengkol'){
                $query = $giziDB->query("SELECT userid, tanggalPenimbangan as submissiondate,count(*) as counts from ".$table." where (userid='gizi8' or userid='gizi9' or userid='gizi10' or userid='gizi11' or userid='gizi12' or userid='gizi13' or userid='gizi14') AND tanggalPenimbangan > '$start' AND tanggalPenimbangan <= '$end'  group by userid, tanggalPenimbangan");
            }
            elseif($kecamatan=='Janapria'){
                $query = $giziDB->query("SELECT userid, tanggalPenimbangan as submissiondate,count(*) as counts from ".$table." where (userid='gizi1' or userid='gizi2' or userid='gizi3' or userid='gizi4' or userid='gizi5' or userid='gizi6') AND tanggalPenimbangan > '$start' AND tanggalPenimbangan <= '$end'  group by userid, tanggalPenimbangan");
            }
            foreach ($query->result() as $datas){
                if(array_key_exists($datas->userid, $users)){
                    $data_count                  = $result_data[$users[$datas->userid]];
    ...
    public function getCountPerFormByVisitDateForDrill($desa="",$date=""){
        ...
        foreach ($tables as $table=>$legend){
            //query tha data
            reset($users);
            $query3 = $giziDB->query("SELECT userid, tanggalPenimbangan as submissiondate,count(*) as counts from ".$table." where (userid='".key($users)."') and tanggalPenimbangan='".$date."' group by userid, tanggalPenimbangan");
            foreach ($query3->result() as $datas){
                if(array_key_exists($datas->userid, $users)){
                    $data_count                  = $result_data[$date];
                    if(array_key_exists($table, $table_default)){
